...
% 
    ...
    for(ind=1:nos)
        ...
        end %                                                    aca decia integral en vezz de quadgk                                                     aca decia inf en vez de 20!
        prices(ind) =  Ralpha(F(ind), K(ind), alphas(ind))+1/pi*quadgk(@(x) phi(x, K(ind), alphas(ind), F(ind), kappa, theta, rho, sigma, tau(ind), v0) , 0, 10000);
        if (PC(ind)==2)
